...
| E9 | Retained foundation rework | mapped 256-record UBO + DrawIDARB + per-frame command rewrite | persistent texel table + `gl_BaseInstanceARB` + dirty `writeToBuffer` | source/shaderc portability tests; fail-open kept | not measured (no GPU) | async timestamps wired, unread here | n/a | n/a | n/a | Ultima-issued map/unmap/fence = 0 on happy path; driver barriers not observed | persistent indirect + visibility `instanceCount` | ARB BaseInstance on GL and Vulkan shaderc | **REWORK** until RTX 3090 A/B |
